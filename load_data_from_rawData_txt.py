...
train_labels_path = 'data/fashion_train'
eval_labels_path = 'data/fashion_test'
train_img_path =  'data/fashion_train'
eval_img_path = 'data/fashion_test'
...
def clean_train_labels():
    train_Y = list()
    filename = train_labels_path + '/fashion_train.txt'
...
def cleaned_test_labels():
    test_Y = list()
    filename = eval_labels_path + '/fashion_test.txt'
...
def sentenceLoader():
    ...
    dump(trainY, open('data_generated/featuresTrainY.pkl', 'wb'))
    dump(testY, open('data_generated/featuresTestY.pkl', 'wb'))

    trainY = np.array(trainY)
    testY = np.array(testY)
    trainY = np.resize(trainY, (-5400,))
    testY = np.resize(testY, (-3141,))
